...
class LAdaBelief(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(LAdaBelief, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(LAdaBelief, self).__setstate__(state)
            ...
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    state['exp_avg'] = torch.zeros_like(p.data,
                                    memory_format=torch.preserve_format) if version_higher else torch.zeros_like(p.data)
                    # Exponential moving average of squared gradient values
                    state['exp_avg_var'] = torch.zeros_like(p.data.view(1,-1)[0,0],
                                    memory_format=torch.preserve_format) if version_higher else torch.zeros_like(p.data.view(1,-1)[0,0])
                    #print(state['exp_avg_var'].size())
                    #input()
                    if amsgrad:
                        # Maintains max of all exp. moving avg. of sq. grad. values
                        state['max_exp_avg_var'] = torch.zeros_like(p.data,
                ...
                #exp_avg_var.mul_(beta2).add_(1 - beta2, torch.mean(torch.pow(grad_residual,2))*torch.ones_like(p.data,
                #                    memory_format=torch.preserve_format))

                mean_grad_residual = torch.mean(torch.pow(grad_residual,2))
                exp_avg_var.mul_(beta2).add_(1 - beta2, mean_grad_residual)
                ...
                if not self.rectify:
                    ...
                    p.data.add_(-step_size/denom, exp_avg)
