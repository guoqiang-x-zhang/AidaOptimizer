...
def get_parser():
    ...
    parser.add_argument('--model', default='resnet', type=str, help='model',
                        choices=['resnet', 'densenet', 'vgg', 'mlp'])
    parser.add_argument('--optim', default='sgd', type=str, help='optimizer',
                        choices=['aida', 'ladabelief','ladam', 'sgd', 'adam',  'adamw', 'adabelief', 'yogi', 'msvag', 'radam', 'fromage'])
...
def get_ckpt_name(model='resnet', optimizer='sgd', lr=0.1, final_lr=0.1, momentum=0.9,
    ...
    name = {
        ...
        'ladam': 'lr{}-betas{}-{}-eps{}-wdecay{}-run{}'.format(lr, beta1, beta2, eps, weight_decay, run),
        'ladabelief': 'lr{}-betas{}-{}-eps{}-wdecay{}-run{}'.format(lr, beta1, beta2, eps, weight_decay, run),
        'aida': 'lr{}-betas{}-{}-eps{}-wdecay{}-run{}'.format(lr, beta1, beta2, eps, weight_decay, run),
    }[optimizer]
...
def train(net, epoch, device, data_loader, optimizer, criterion, args):
    ...
    print('train loss %.5f acc %.3f'%(train_loss, accuracy))

    logging.info("Training - epoch: %d loss: %.10f training %.4f", epoch, train_loss, accuracy)
...
def test(net, device, data_loader, criterion):
    ...
    accuracy = 100. * correct / total
    print(' test loss %.5f, acc %.3f'%(test_loss, accuracy))
    logging.info("Test - epoch: loss: %.10f training %.4f", test_loss, accuracy)

    return accuracy

def adjust_learning_rate(optimizer, epoch, step_size=150, gamma=0.1, reset = False):
    for param_group in optimizer.param_groups:
        if (epoch % 100==0 or epoch % 160==0) and epoch>0:
...
def main():
    ...
    logging.basicConfig(filename=os.path.join(".", "results", args.model+"_optim_"+args.optim \
        +"_test_lr_"+str(args.lr)+"_beta2_"+str(args.beta2)
        +"_decay_"+str(args.weight_decay)+"_eps_"+eps_str+"_"+str(args.run)+".txt"), filemode='w', level=logging.INFO)
